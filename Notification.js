class NotificationManager {
    ...
    constructor(apiUrl, userManager) {
        ...
        // On cible le TBODY dÃƒÂ©fini dans l'index.html
        this.tbody = document.getElementById("maintenanceList");
        
        if (this.tbody) {
            // On lance le polling (vÃƒÂ©rification pÃƒÂ©riodique)
            this.initPolling();
            // On lance l'ÃƒÂ©coute des WebSockets pour le temps rÃƒÂ©el
            this.initSocket();
        }
    }
    ...
    initSocket() {
        if (typeof io !== 'undefined') {
            ...
            // DÃƒÂ¨s qu'une prise passe en 'hs' (ou en 'libre'), on rafraÃƒÂ®chit les alertes
    ...
    initPolling() {
        // Essai immÃƒÂ©diat (si dÃƒÂ©jÃƒ  connectÃƒÂ©)
        this.fetchAlerts();
        // Puis toutes les 10 secondes pour avoir du temps rÃƒÂ©el
        setInterval(() => this.fetchAlerts(), 10000);
    }

    async fetchAlerts() {
        // Si on n'est pas connectÃƒÂ©, on ne spamme pas l'API pour rien
    ...
    updateUI(alerts) {
        ...
        if (!alerts || alerts.length === 0) {
            this.tbody.innerHTML = `<tr><td colspan="2" style="text-align:center; color:#27ae60;">Aucune alerte Ã¢Å“â€¦</td></tr>`;
            return;
        }
        ...
        alerts.forEach(alert => {
            ...
            // Date formatÃƒÂ©e (Heure seulement pour gagner de la place)
...
document.addEventListener("DOMContentLoaded", () => {
    // On passe l'URL rÃƒÂ©elle et l'instance userManager existante
